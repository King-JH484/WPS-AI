...
| Complete | B5 Mac 真机与 Windows 探针交接 | 平台证据报告、Windows 执行指南 | Mac WPS 人工流程 + 报告字段校验 |
| Complete | C1 原生图表（代码层） | 柱状、折线、饼/环、散点的创建/读取/更新；真机状态由写探针决定 | `node --test test/wpp-native-chart.test.js`；Mac WPS 人工检查仍属 B5 |
| Complete | C2 回归与发布验证 | 完整测试、lint、静态扫描、build、最终报告 | 下列 Verification 全部完成或明确记录环境限制 |
...
- 2026-08-31：完成 Mac 12.1.25867 真机受控写探针；母版固定形状、自定义版式、按版式加页和原生图表对象闭环通过，占位符与 ChartData Workbook 在有限延迟复测后仍按 degraded 处理；Windows 保持待目标机探测。
- 2026-08-31：完成发布回归；新增相关 31 项测试全过，完整测试 740 项中 734 通过、5 项既有基线失败、1 跳过；变更文件 ESLint 与 Windows 静态门禁通过。全仓 lint 仍被既有 `js/wps.js` 重复声明阻断，Semgrep 模块与 `wpsjs` CLI 在本机不可用，均记录为环境/仓库基线限制。
